...
class Runner:
    ...
    def train(self):
        ...
        for iter_i in tqdm(range(res_step+1)):
            ...
            # 1. 准备数据 (开启梯度以支持二阶导数计算)
            samples, nn = self.dataset_global.global_train_data(batch_size)
            samples.requires_grad = True 

            # 2. 计算梯度和移动
            gradients_sample = self.global_field.gradient(samples).squeeze() 
            udf_sample = self.global_field.udf(samples)
            ...
            sub_index = np.random.choice(batch_size, 100, replace=False)
            sub_sampled = sample_moved[sub_index, :].clone()
            knn, sub_sampled_knn, fur = self.dataset_global.get_knn(sub_sampled, sub_sampled)
            
            # Local Network 推理 (这里会计算 PCA 法线)
            dis = self.local_network(knn, sub_sampled_knn) * fur
            local_loss = dis.mean()
            ...
            # 5. 日志记录 (✨✨✨ 关键修复: 使用 .item() 防止图残留 ✨✨✨)
            if self.iter_step % self.report_freq == 0:
                print_log('iter:{:8>d} all = {:.6f} local_loss = {:.6f} np_loss = {:.6f} surf_loss = {:.6f} sp_loss = {:.6f} lr={:.6f}'.format(
                    self.iter_step, 
                    loss.item(),        # 必须是数值
                    local_loss.item(),  # 必须是数值
                    np_loss.item(),     # 必须是数值
                    surf_loss.item(),   # 必须是数值
                    sp_loss.item(),     # 必须是数值
                    self.optimizer.param_groups[0]['lr']), logger=logger)

            # 6. Upsample 验证
            if self.iter_step % self.val_freq == 0:
                # 显式清理缓存
                del loss, local_loss, global_loss, samples, gradients_sample
                ...
                if self.iter_step % self.save_freq == 0: 
                    self.save_checkpoint()
                continue # 跳过下面的 del，因为已经 del 过了
            ...
            # 7. ✨✨✨ 显式清理本轮变量 ✨✨✨
            del loss, local_loss, global_loss, np_loss, surf_loss, sp_loss
            del samples, gradients_sample, udf_sample, grad_norm, sample_moved
            del sub_sampled, knn, sub_sampled_knn, dis, fur, nn
    ...
    def upsample(self, epoch=None):
        ...
        for i in range(0, pointcloud.shape[0], self.batch_size):
            ...
            with torch.no_grad():
                filter_udf = self.global_field(query).reshape(-1)
                valid_query = query[filter_udf<=surf_max]
                results.append(valid_query.detach().cpu().numpy())
            
            # 循环内清理
            del grad, dis, query, filter_udf, valid_query, partial
    ...
    def upsample_seed(self, epoch=None):
        # 保持与 upsample 相同的逻辑，加上 detach 和 del 即可
        # (为节省篇幅，这里复用 upsample 的修复逻辑)
        from global_models.local import normalize_point_cloud
        from einops import rearrange
        pointcloud = self.dataset_global.pointcloud.clone()
        surf_max = self.global_field.udf(pointcloud).max()
        ...
        for i in tqdm(range(0, samples.shape[0], self.batch_size)):
            ...
            del grad, dis, query, valid_query
    ...
    def load_local_checkpoint(self, dataset):
        use_normal_estimation = self.conf.get_bool('general.use_normal_estimation', default=True)
        self.local_network = get_local_model(dataset, use_normal_estimation).cuda()
        ckpt_path = self.conf['general.local_path']
        print("=" * 70)
        print(f"Loading local checkpoint from: {ckpt_path}")
